...
router.post('/', async (req, res) => {
  ...
  const vehs = await db.query('SELECT price FROM vehicles WHERE slug = ? AND active = 1', [b.vehicleSlug]);
  const veh = vehs[0];
  ...
  `, [b.vehicleSlug, b.startDate, b.endDate]);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const exists = await db.query('SELECT 1 FROM reservations WHERE booking_id = ?', [bookingId]);
